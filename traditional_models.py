...
def load_and_preprocess_data(data_path):
    ...
    feature_cols = [
        ...
        'average_wind_speed', 'dow', 'year', 'month', 'weekday',
        'weekday_non_holiday', 'dt', 'season'
    ]
    ...
    train_target = target_scaler.fit_transform(train_data[['trips']])
    test_target = target_scaler.transform(test_data[['trips']])
...
def moving_average(data, window=7):
    """简单移动平均预测"""
    ma = data['trips'].rolling(window=window).mean()
    return ma.iloc[window-1:].values

def exponential_smoothing(data, alpha=0.3):
    """指数平滑预测"""
    result = [data['trips'].iloc[0]]
    for n in range(1, len(data)):
        result.append(alpha * data['trips'].iloc[n] + (1 - alpha) * result[n-1])
...
def main():
    # 加载数据
    data_path = './data/daily_citi_bike_trip_counts_and_weather.csv'
    ...
    metrics = evaluate_model(test_data['trips'].values, ma_pred, 'Moving Average')
    results['Moving Average'] = metrics
    plot_predictions(test_data['trips'].values, ma_pred, metrics, 'Moving Average', 
                    os.path.join(save_dir, 'ma_predictions.png'))
    
    # 2. 指数平滑法
    es_pred = exponential_smoothing(pd.concat([train_data, test_data]))[-len(test_data):]
    metrics = evaluate_model(test_data['trips'].values, es_pred, 'Exponential Smoothing')
    results['Exponential Smoothing'] = metrics
    plot_predictions(test_data['trips'].values, es_pred, metrics, 'Exponential Smoothing',
                    os.path.join(save_dir, 'es_predictions.png'))
    
    # 3. Holt-Winters
    hw_pred = holt_winters(pd.concat([train_data, test_data]))[-len(test_data):]
    metrics = evaluate_model(test_data['trips'].values, hw_pred, 'Holt-Winters')
    results['Holt-Winters'] = metrics
    plot_predictions(test_data['trips'].values, hw_pred, metrics, 'Holt-Winters',
    ...
    metrics = evaluate_model(test_data['trips'].values, lr_pred, 'Linear Regression')
    results['Linear Regression'] = metrics
    plot_predictions(test_data['trips'].values, lr_pred, metrics, 'Linear Regression',
